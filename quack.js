...
async function collectFromList(token, ua, listNests, listDucks) {
  ...
  if (data) {
    ...
    console.log(`ĐÃ THU HOẠCH [ NEST 🌕 ${listNests[0].id} ]`);
...
async function harvestAllEgg() {
  try {
    ...
    // console.log(wallets);
    console.log("LINK TOOL : [ IB TELE :@LONGNGUYENVANMEDIA ĐỂ LẤY ]");
    console.log("CRE : [ LONG NG VĂN ]");
    console.log(
      `THỜI GIAN CHẠY : [ ${timerInstance
        .getTimeValues()
        .toString(["days", "hours", "minutes", "seconds"])} ]`
    );
    console.log(`TỔNG THU HOẠCH : [ ${eggs} EGG 🥚]`);
    console.log("---");
    ...
    if (timeToGoldenDuck <= 0) {
      ...
      if (data.time_to_golden_duck === 0) {
        console.log("[ GOLDEN DUCK 🐥 ] : BOSS XUẤT HIỆN");
    ...
    console.log(`[ GOLDEN DUCK 🐥 ] : ${timeToGoldenDuck}s nữa gặp`);
